...
'''
Assuming zero covariances between data, this script computes the 1dmax, median,
and +/- 1 sigma optical depth values for each pixel in the galaxy image. Maps 
are saved as numpy arrays. The output_map input should be set manually.  
...
def prob1d_maps(x,y):  
    ...
    if (np.isnan(chi2)).all():
        ...
        if two_comp:
            tau = np.load('radex_model/tau_'+model+'_'+output_map+'.npy').reshape(-1)
        else:
            tau = np.load('radex_model/tau_'+model+'_'+output_map+'.npy').reshape(-1)
            tau[mask == 0] = np.nan
        tau = np.log10(tau)
        ...
        np.save(sou_model+source+'_prob1d/tau_'+output_map+'_'+model+'_'+str(x)+'_'+str(y)+'.npy', counts_norm)
        ## 1DMax
...
for result in results:
    ...
    np.save(sou_model+'tau_'+model+'_'+source+'_'+output_map+'_1dmax_los200.npy', map_1dmax)
    np.save(sou_model+'tau_'+model+'_'+source+'_'+output_map+'_neg1sig_los200.npy', map_neg1sig)
    np.save(sou_model+'tau_'+model+'_'+source+'_'+output_map+'_median_los200.npy', map_median)
    np.save(sou_model+'tau_'+model+'_'+source+'_'+output_map+'_pos1sig_los200.npy', map_pos1sig)
