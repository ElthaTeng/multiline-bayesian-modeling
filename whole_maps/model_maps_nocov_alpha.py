...
if two_comp:
    ...
    if output_map == 'alpha32':
        model_co32 = np.load(sou_model+'flux_'+model+'_co32.npy')
...
def prob1d_maps(x,y):  
    ...
    if (np.isnan(chi2)).all():
        ...
        if two_comp:
            alpha = ((10**Nco_0 * 10**phi_0 +  10**Nco_1 * 10**phi_1) 
                    / model_co10.reshape(-1) * Xco2alpha).reshape(-1)
        else:
            alpha = 10**Nco * 10**phi / model_co10.reshape(-1) * mask * Xco2alpha
            alpha[mask == 0] = np.nan
...
np.save(sou_model+'Xco_'+model+'_'+source+'_'+output_map+'_1dmax_los200.npy', map_1dmax)
np.save(sou_model+'Xco_'+model+'_'+source+'_'+output_map+'_neg1sig_los200.npy', map_neg1sig)
np.save(sou_model+'Xco_'+model+'_'+source+'_'+output_map+'_median_los200.npy', map_median)
np.save(sou_model+'Xco_'+model+'_'+source+'_'+output_map+'_pos1sig_los200.npy', map_pos1sig)
